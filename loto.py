...
class Pole:
    def __init__(self, zn=0): #значение и флаг поля
        self.zn = zn
        self.fl = True
    ...
    def prn(self):
        if self.zn > 0: return self.zn if self.fl else '-'
        else: return ''

    def vich(self): #вычеркивание карточки
        self.fl = False
...
class Cart:
    ...
    def gen_cart(self):
        ...
        for i in range(3):
            ...
            for n in range(9):
                if n in des:
                    self.cart.append(Pole(line[des.index(n)]))
                else:
                    self.cart.append(Pole())
